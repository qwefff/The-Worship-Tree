...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		let gain = tmp[layer].baseAmount.div(tmp[layer].requires).div(tmp[layer].gainMult).max(1).log().div(ExpantaNum(tmp[layer].base).log()).times(tmp[layer].gainExp).pow(OmegaNum.pow(tmp[layer].exponent, -1))
		
		// return tmp[layer].baseAmount.div(tmp[layer].requires).div(tmp[layer].gainMult).max(1)
		// return tmp[layer].baseAmount.div(tmp[layer].requires).div(tmp[layer].gainMult).max(1).log(200)
		// return new ExpantaNum(tmp[layer].base).pow(tmp[layer].baseAmount.div(tmp[layer].requires).div(tmp[layer].gainMult).max(1).log(tmp[layer].base))
